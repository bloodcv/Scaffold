module.exports = {
  ...
  rules: {
    ...
    "curly": ["error", "all"], // 强制使用花括号的风格（强制所有控制语句使用一致的括号风）
    "default-case": 2, // 在switch语句中需要有default语句
    "dot-notation": ["error", {"allowKeywords": false, "allowPattern": "^[a-z]+(_[a-z]+)+$"}], // 获取对象属性的时候使用点号
    "eqeqeq": ["error", "smart"], // 比较的时候使用严格等于(要求使用 === 和 !==)
    ...
    "no-return-assign": 2, // 不允许在return语句中使用分配语句(禁止在 return 语句中使用赋值语句)
    ...
    "no-use-before-define": ["error", { "functions": true, "classes": true }], // 不允许在未定义之前就使用变量"indent": 2, //强制一致的缩进风格
    ...
    "quotes": [1, "single", { "allowTemplateLiterals": true }], // 引号风格
    "semi": [0, "always"], // 强制语句分号结尾
    "semi-spacing": [0, {"before": false, "after": false}], // 分后前后空格
    "sort-vars": 0, // 变量声明时排序
    "space-before-blocks": [0, "always"], // 块前的空格
    "space-before-function-paren": [2, {"anonymous": "always", "named": "never", "asyncArrow": "always"}], // 函数定义时括号前的空格
